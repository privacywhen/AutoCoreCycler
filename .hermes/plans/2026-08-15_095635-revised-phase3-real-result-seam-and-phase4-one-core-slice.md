...
| parser/error evidence attributable to the selected core/candidate under the verified-idle-host model | `ATTRIBUTED_FAIL` |
| WHEA/process/CPU/load/parser evidence lacking sufficient attribution | `AMBIGUOUS_FAIL` |
| script/adapter/config/log/protocol lifecycle failure or unclassifiable observation | `INFRASTRUCTURE_INVALID` |
...
Do not broaden cleanup to host-wide process discovery. The user accepts CoreCycler logs/events under a verified-idle-host evidence model, but recovery durability and reader integrity remain mandatory; use that only if the real lifecycle establishes it as the necessary source.
...
- attributable CoreCycler parser/lifecycle error;
- ambiguous WHEA/process/CPU evidence;
- malformed/missing evidence.

For valid controlled completion, prove resulting `PASS` passes `Test-DiscoveryStageResult` only with matching context and verified lifecycle fields. For a controlled attributable error, prove `ATTRIBUTED_FAIL` reaches `Resolve-DiscoverySuiteStage`. This is not a hardware PASS; it is proof the real production result seam maps its own parser/lifecycle semantics correctly.
...
- controlled fixtures prove `PASS`, `ATTRIBUTED_FAIL`, `AMBIGUOUS_FAIL`, and `INFRASTRUCTURE_INVALID` mapping through that same seam;
...
Start a state for one core with candidate `-18`, a defined required ordered suite (e.g. `@('kagari', 'prime95-720k')`), and fresh distinct candidate/stage IDs. Use:
...
→ APPLIED_VERIFIED
→ controlled real-result seam PASS for kagari
→ ADVANCE_STAGE only to prime95-720k
→ controlled real-result seam PASS for prime95-720k
