...
It 'creates combined-map validation phases from candidate recommendations' {
    $recommendation = Get-CandidateRecommendationFixture
    $phase = New-CoreCyclerCombinedMapValidationPhase -Name 'combined-map-validation' -CandidateMap $recommendation -RyzenGeneration Ryzen7000 -ExpectedCoreCount 4
    $ini = ConvertTo-CoreCyclerIni -Phase $phase

    Assert-ArrayEqual -Actual $phase.CandidateMap -Expected @(-18, -15, -13, -26)
    Assert-MatchText $ini '(?m)^# PhaseType = CombinedMapValidation\r?$'
    Assert-MatchText $ini '(?m)^# CandidateMap = -18, -15, -13, -26\r?$'
    Assert-MatchText $ini '(?m)^# MapApplication = Candidate map must already be active before validation; with enableAutomaticAdjustment = 0, CoreCycler will not apply startValues\.\r?$'
    ...
    if ($ini -Match '(?m)^enableAutomaticAdjustment = 1\r?$') {
        throw('Combined-map validation must not enable automatic adjustment by default.')
    }

    if ($ini -Match '(?m)^setVoltageOnlyForTestedCore = 1\r?$') {
        throw('Combined-map validation must keep all candidate CO values active together.')
    }
}

It 'matches the combined-map validation fixture' {
    $recommendation = Get-CandidateRecommendationFixture
    $phase = New-CoreCyclerCombinedMapValidationPhase -Name 'combined-map-validation' -CandidateMap $recommendation -RyzenGeneration Ryzen7000 -ExpectedCoreCount 4
    $ini = ConvertTo-CoreCyclerIni -Phase $phase
    $fixture = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'fixtures\combined-map-validation.ini') -Raw

    Assert-SameNormalizedText -Actual $ini -Expected $fixture
}

It 'forces automatic adjustment off even when overrides try to enable it' {
    $phase = New-CoreCyclerCombinedMapValidationPhase -Name 'safe-combined' -CandidateMap @(-5, -4, -3, 0) -Sections @{
        General = @{
            runtimePerCore = '45m'
        }
        AutomaticTestMode = @{
    ...
    Assert-MatchText $ini '(?m)^runtimePerCore = 45m\r?$'
    Assert-MatchText $ini '(?m)^enableAutomaticAdjustment = 0\r?$'
...
It 'rejects positive candidate maps by default' {
    Assert-Throws { New-CoreCyclerCombinedMapValidationPhase -CandidateMap @(-5, 1, 0, -2) } 'Positive Curve Optimizer value found'
}

It 'requires positive CO opt-in at generation and INI conversion time' {
    $phase = New-CoreCyclerCombinedMapValidationPhase -Name 'explicit-positive-combined' -CandidateMap @(-5, 1, 0, -2) -AllowPositiveCurveOptimizer
...
It 'rejects candidate maps that do not match an expected core count' {
    Assert-Throws { New-CoreCyclerCombinedMapValidationPhase -CandidateMap @(-5, -4, -3) -ExpectedCoreCount 4 } 'expected 4'
}

Write-Host ('combined-map-validation.Tests.ps1: ' + $Script:TestCount + ' tests passed.')
